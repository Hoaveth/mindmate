...
function LoginPage() {
	...
	const authFormik = useFormik({
		...
		onSubmit: async (values) => {
			...
			setLocalStorageItem(USER_KEY, response.user)
			setError('')
			router.push('/')
		}
	})
		...
		<div className="flex items-center justify-center mt-8">
			...
			>
				...
				<div className="mb-4 mt-5">
					...
					<input
						ref={emailRef}
						className={`bg-transparent border rounded w-full py-2 px-3  leading-tight focus:outline-none focus:shadow-outline ${authFormik.touched.email &&
						authFormik.errors.email ? 'border border-red-500' : ''}`}
						type="email"
						name="email"
						onChange={authFormik.handleChange}
						vale={authFormik.values.email}
					/>
					{
				...
				<div className="mb-6">
					...
					<input
						className={`bg-transparent border rounded w-full py-2 px-3  leading-tight focus:outline-none focus:shadow-outline ${authFormik.touched.password &&
							authFormik.errors.password ? 'border border-red-500' : ''}`}
						type="password"
						name="password"
						onChange={authFormik.handleChange}
						vale={authFormik.values.password}
					/>
					{
